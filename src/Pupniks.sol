...
error SetSignerBeforeTogglingSale();
...
contract Pupniks is ERC721, Ownable {
     ...
     * @dev The nonce is used to prevent replay attacks. There is a maximum amount of 3000 pupniks that can be minted. 
     *
     ...
     * @param signature The signature of the message.
     * @param nonce The nonce used to sign the message.
     * @param ids The array of tokenIds to mint.
     */
     function mintSpecificPupniks(bytes32 hash, bytes calldata signature, uint256 nonce, uint256[] calldata ids) external payable {
        _requireSaleOpen();
        _requireValidSignature(hash, signature, nonce, ids.length);
        _useNonce(msg.sender, nonce);

        uint256 currentAmount = amountMinted;
        if (currentAmount < TOTAL_SUPPLY) {
            revert CannotMintSpecificIdsUntilAllMinted();
        }
    ...
    function toggleSaleStatus() external onlyOwner {
        if (signerAddress == address(0)) {
            revert SetSignerBeforeTogglingSale();
        }
        saleLive = !saleLive;
     ...
     * @dev This function will revert if the token does not exist.
     *
    ...
    function _requireValidSignature(bytes32 hash, bytes calldata signature, uint256 nonce, uint256 quantity)
        ...
        view
    {
        if (signerAddress != hash.recover(signature)) {
            revert InvalidSignature();
        }

        bytes32 localHash = keccak256(
            abi.encodePacked(
                "\x19Ethereum Signed Message:\n32", keccak256(abi.encodePacked(msg.sender, nonce, quantity))
            )
        );
    ...
    function _requireMetadataNotLocked() private view {
        if (metadataLocked) {
            revert ContractMetadataLocked();
        }
    }
